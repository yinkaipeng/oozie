...
public class TestCoordActionStartXCommand extends XDataTestCase {
    ...
    protected Configuration getCoordConf(Path coordAppPath) throws IOException {
        ...
        jobConf.setProperty("wfAppPath", new Path(wfAppPath.toString(), "workflow.xml").toString());
        jobConf.setProperty("mrclass", MapperReducerForTest.class.getName());
        jobConf.setProperty("delPath", new Path(wfAppPath.toString(), "output").toString());
        jobConf.setProperty("subWfApp", new Path(wfAppPath.toString(), "subwf/workflow.xml").toString());
